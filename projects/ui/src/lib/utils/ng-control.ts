...
export function trackFormControl<T = any>(thisObj: any, options: { attachValueAccessor?: boolean } = {}) {
  return new (class {
    ...
    init() {
      ...
      if (ngControl) {
        if (options.attachValueAccessor ?? true) {
          if (!ngControl.valueAccessor || (thisObj && thisObj instanceof (ngControl.valueAccessor as any).constructor)) {
            ngControl.valueAccessor = thisObj as unknown as ControlValueAccessor;
          }
        }

...
//! picker directives
@Directive({ selector: 'ard-color-input > ng-template[ard-shade-indicator-tmp]' })
...
@Directive({ selector: 'ard-color-input > ng-template[ard-opacity-indicator-tmp]' })
export class ArdColorInputOpacityIndicatorTemplateDirective {
    constructor(public template: TemplateRef<ColorPickerIndicatorContext>) { }
}

@Directive({ selector: 'ard-color-input > ng-template[ard-color-reference-tmp]' })
...
//! action buttons
@Directive({ selector: 'ard-color-picker > ng-template[ard-action-buttons-tmp]' })
